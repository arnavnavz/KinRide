...
interface RideHistoryItem {
  ...
  ratings: { stars: number; comment: string | null; riderId: string }[];
  tips: { amount: number; riderId: string }[];
}
...
export default function DriverHistoryPage() {
  ...
  const totalEarned = completedRides.reduce((sum, r) => {
    const fare = r.estimatedFare || 0;
    const fee = r.platformFee || 0;
    return sum + (fare - fee);
  }, 0);
  const totalTips = rides.reduce(
    (sum, r) => sum + r.tips.reduce((ts, tip) => ts + tip.amount, 0),
    0
  );
  const avgRating =
    completedRides.length > 0
      ? completedRides
          .flatMap((r) => r.ratings)
          .reduce((sum, r, _, arr) => sum + r.stars / arr.length, 0)
      : 0;
      ...
      <div className="max-w-2xl mx-auto p-4 sm:p-6 pb-24 space-y-5 animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {total} {total === 1 ? "ride" : "rides"} completed
            </p>
          </div>
        </div>
        ...
        {total > 0 && (
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-3 text-center">
              <p className="text-lg font-bold text-primary">${totalTips.toFixed(2)}</p>
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-12 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {filter ? "Try a different filter" : "Your completed rides will appear here"}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {rides.map((ride) => {
              const earnings = (ride.estimatedFare || 0) - (ride.platformFee || 0);
              const tipTotal = ride.tips.reduce((s, t) => s + t.amount, 0);
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-800">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-400">{ride.rider.name}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      {tipTotal > 0 && (
                        <span className="text-xs text-green-500 font-medium">+${tipTotal.toFixed(2)} tip</span>
                      )}
                      {ride.status === "COMPLETED" && (
                        <span className="text-sm font-bold text-green-600">${earnings.toFixed(2)}</span>
                      )}
                    </div>
                  </div>
                </Link>
